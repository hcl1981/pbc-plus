...
static void fatal_screen(const char *what, const char *detail, uint32_t addr)
{
    ...
    pbc_display_text(20,  24, &segfont_l, "NOIZ2SA", COL_HL, COL_BG);
    pbc_display_text(20,  64, &segfont_m, what, COL_FG, COL_BG);
    if (detail && detail[0])
        pbc_display_text(20,  94, &segfont_s, detail, COL_FG, COL_BG);
    pbc_display_text(20, 120, &segfont_s, "ADDR", COL_HL, COL_BG);
    hex32(buf, addr);
    pbc_display_text(20, 138, &segfont_l, buf, COL_FG, COL_BG);
    pbc_display_text(20, 190, &segfont_s, "ARM-NONE-EABI-ADDR2LINE -E", COL_FG, COL_BG);
    pbc_display_text(20, 206, &segfont_s, "NOIZ2SA.ELF -F -C ADDR", COL_FG, COL_BG);
    pbc_display_text(20, 240, &segfont_s, "BOOTSEL HALTEN UND USB STECKEN", COL_HL, COL_BG);
