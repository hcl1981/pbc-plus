...
ConnecTile(short x, short y, short *TileAdrPtr, short Command)
{
  ...
  if ((Command >= 2) && (Command <= 4)) {
    /* only auto-bulldoze if we can afford the doze ($1) AND the build that
       follows, so we never clear a tile we then can't pay to build on */
    short need = (Command == 2) ? 11 : (Command == 3) ? 21 : 6;  /* road / rail / wire */
