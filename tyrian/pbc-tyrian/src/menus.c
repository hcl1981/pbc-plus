...
#ifdef WITH_NETWORK
/* PicoBoy: im Spielmodus-Menue wurde "Network Game" gewaehlt. newGame() fragt
   daraufhin die Rolle ab und baut die Verbindung auf. */
...
bool gameplaySelect(void)
{
	...
	for (; ; )
	{
...
#ifdef WITH_NETWORK
			/* PicoBoy: Netzspiel ueber das USB-C-Kabel zwischen zwei Geraeten
			   ist hier moeglich, der Punkt bleibt also waehlbar. */
		...
		if (action)
		{
			...
			{
#ifdef WITH_NETWORK
				/* PicoBoy: Rolle (Gastgeber/Beitreten) wird gleich danach in
				   newGame() abgefragt -- erst dort steht fest, dass wirklich
				   ein Netzspiel gestartet wird. */
				JE_playSampleNum(S_SELECT);
...
bool episodeSelect(void)
{
	...
	const int xCenter = 320 / 2;
	const int yMenuHeader = 20;
	/* PicoBoy: an den sichtbaren Ausschnitt angepasst -- bei 20 fiel der
	   Anfang der Titel weg. Siehe PBC_MENU_SAFE_X in platform/pbc_video.h. */
	const int xMenuItem = PBC_MENU_SAFE_X;
