...
void load_sprites(unsigned int table, FILE *f)
{
	...
	for (unsigned int i = 0; i < sprite_table[table].count; ++i)
	{
		...
		/* PicoBoy: kopierfrei. Die Sprite-Daten stehen bereits gelesen im
		   Adressraum (Flash ab 0x10000000); sie noch einmal ins RAM zu holen
		   waere bei tyrian.shp allein rund 440 KB, die es nicht gibt. */
		cur_sprite->data = (Uint8 *)xipfs_inplace(f, cur_sprite->size);
...
void free_sprites(unsigned int table)
{
	for (unsigned int i = 0; i < sprite_table[table].count; ++i)
	{
		...
		/* PicoBoy: nach dem kopierfreien Laden zeigt data ins Flash --
		   das darf nicht an free(). */
		if (!xipfs_owns(cur_sprite->data))
...
void JE_loadCompShapesB(Sprite2_array *sprite2s, FILE *f)
{
	...
	/* PicoBoy: kopierfrei, siehe load_sprites. */
	sprite2s->data = (Uint8 *)xipfs_inplace(f, sprite2s->size);
...
{
	/* PicoBoy: siehe free_sprites. */
	if (!xipfs_owns(sprite2s->data))
		free(sprite2s->data);
	sprite2s->data = NULL;
