...
/* --- Load Level/Map Data --- */
/*
 * PicoBoy-Aenderung: diese Funktion hatte 76 KB an lokalen Feldern.
 *
 * Auf einem PC mit Megabyte-Stapel faellt das nicht auf. Hier teilt sich der
 * Stapel den freien Speicher mit dem Heap, und das sind zusammen rund 40 KB --
 * die Funktion schrieb also weit in die Spieldaten hinein. Nach aussen sah das
 * aus wie ein schwarzer Bildschirm beim Levelstart.
 *
 * pic_buffer ist nur eine Momentaufnahme des Bildschirms fuer die
 * Uebergangseffekte am Levelanfang, und Tyrian haelt ohnehin drei
 * 320x200-Flaechen. Die Uebergaenge brauchen davon zwei gleichzeitig
 * (VGAScreen als Ziel, VGAScreen2 als altes Bild) -- die dritte ist frei und
 * genau so gross.
 *
 * Welche das ist, haengt davon ab, worauf VGAScreen gerade zeigt: waehrend des
 * Spiels ist das game_screen, sonst VGAScreenSeg. Die Auswahl unten trifft
 * deshalb immer die verbleibende. VGAScreen2 kommt nie in Frage -- sie wird in
 * denselben Effekten gebraucht.
 *
 * Ihr Inhalt darf verloren gehen: danach beginnt das Level und zeichnet jedes
 * Bild vollstaendig neu.
 */
#define PIC_BUFFER_BYTES (320 * 200)

void JE_loadMap(void)
{
	JE_word x, y;
	JE_integer yy;
	/* PicoBoy: zusammen 2,3 KB -- statisch statt auf dem Stapel. */
	static JE_word mapSh[3][128]; /* [1..3, 0..127] */
	static JE_byte *ref[3][128]; /* [1..3, 0..127] */
	char s[256];

	/*
	 * Die Kartenebenen werden nur gelesen und liegen bereits im Adressraum --
	 * ein 9-KB-Puffer dafuer waere weder auf dem Stapel noch statisch zu
	 * bezahlen. mapBuf zeigt deshalb direkt ins Flash.
	 */
	const JE_byte *mapBuf;
	JE_word bufLoc;

	char buffer[256];
	int i;
	Uint8 *const pic_buffer = (Uint8 *)(VGAScreen == game_screen
	                                    ? VGAScreenSeg->pixels
	                                    : game_screen->pixels);
...
new_game:
				...
				if (s[0] == ']')
				{
					...
					case 'U':  // Pan up to picture.
						if (!constantPlay)
						{
							...
							JE_word tempX = atoi(s + 3);
							JE_loadPic(VGAScreen, tempX, false);
							memcpy(pic_buffer, VGAScreen->pixels, PIC_BUFFER_BYTES);
							...
							memcpy(VGAScreen->pixels, pic_buffer, PIC_BUFFER_BYTES);
						}
						break;
	...
	/*
	 * PicoBoy-Aenderung: die Kacheln werden nicht mehr ins RAM kopiert.
	 *
	 * shapes*.dat enthaelt je Kachel ein Flag und, wenn sie nicht leer ist,
	 * 672 unkomprimierte Bytes. Weil das Flash in den Adressraum eingeblendet
	 * ist, liefert xipfs_inplace() einen Zeiger genau dorthin und setzt den
	 * Lesezeiger weiter -- ref[][] zeigt danach ins Flash statt in megaData.
	 * Das spart 143 KB, die es auf diesem Geraet schlicht nicht gibt.
	 *
	 * Leere Kacheln teilen sich einen gemeinsamen Nullblock. Das ist
	 * unbedenklich, weil Kacheldaten nach dem Laden nirgends mehr beschrieben
	 * werden.
	 *
	 * Die frueher hier berechneten fill-Werte sind entfallen: sie wurden
	 * gespeichert, aber nie gelesen (siehe varz.h).
	 */
	static const JE_DanCShape blank_shape;   /* dauerhaft null */
	...
	for (int z = 0; z < 600; z++)
	{
		...
		const JE_byte *shape_ptr;
		if (shapeBlank)
		{
			shape_ptr = blank_shape;
		}
		else
		{
			shape_ptr = xipfs_inplace(shpFile, sizeof(JE_DanCShape));
			if (shape_ptr == NULL)
			{
				/* Nur erreichbar, wenn shapes*.dat nicht aus dem Flash-Archiv
				   kommt. Dann waere jede Kachel dieselbe Stelle im Puffer --
				   lieber hier abbrechen als ein falsches Level zeigen. */
				fprintf(stderr, "error: shapes file not memory-mapped\n");
				exit(EXIT_FAILURE);
			}
		}
		...
		{
			if (mapSh[1][x] == z+1)
				ref[1][x] = (x != 71 && !shapeBlank) ? (JE_byte *)shape_ptr : NULL;
		}
		...
		{
			if (mapSh[2][x] == z+1)
				ref[2][x] = (x < 70 && !shapeBlank) ? (JE_byte *)shape_ptr : NULL;
		}
	}

	fclose(shpFile);

	mapBuf = xipfs_inplace(level_f, 14 * 300);
	if (mapBuf == NULL)
	{
		fprintf(stderr, "error: level map not memory-mapped\n");
		exit(EXIT_FAILURE);
	}
	...
	mapBuf = xipfs_inplace(level_f, 14 * 600);
	if (mapBuf == NULL)
	{
		fprintf(stderr, "error: level map not memory-mapped\n");
		exit(EXIT_FAILURE);
	...
	mapBuf = xipfs_inplace(level_f, 15 * 600);
	if (mapBuf == NULL)
	{
		fprintf(stderr, "error: level map not memory-mapped\n");
		exit(EXIT_FAILURE);
	...
	/* PicoBoy: ab hier laeuft das Spielfeld -- Originalpixel statt
	   verkleinertem Gesamtbild. Siehe platform/pbc_video.h. */
	pbc_set_view_mode(PBC_VIEW_GAME);
}
...
bool newGame(void)
{
	...
	{
		/*
		 * PicoBoy: im Spielmodus-Menue wurde "Network Game" gewaehlt. Rolle
		 * abfragen, Verbindung aufbauen -- Episode und Schwierigkeit waehlt
		 * dann der Gastgeber in networkStartScreen() und schickt sie
		 * herueber, der Beitretende bekommt sie zugeteilt.
		 */
		bool netgame = false;
#ifdef WITH_NETWORK
		if (networkRequested)
		...
		else if (twoPlayerMode)
		{
			...
			/* Im Netzspiel hat networkStartScreen() das bereits getan --
			   ein zweites Mal waere zwei Stufen zu schwer. */
			if (!netgame)
				difficultyLevel++;  // Make it one step harder for 2-player mode!
...
{
	/*
	 * PicoBoy-Aenderung: Eintrag 0 ist die beschreibbare Vorlage im RAM, alle
	 * uebrigen liegen im Flash (siehe episodes.h). Nur hier ist die
	 * Unterscheidung noetig -- Panzerung und Grafik der Vorlage werden
	 * ausschliesslich in dieser Funktion gelesen.
	 */
	const JE_EnemyDatEntry *const edat =
		(eDatI == 0) ? &enemyDat_scratch : &enemyDat[eDatI];
	...
	enemy->enemyground = ((*edat).explosiontype & 1) == 0;
	enemy->explonum = (*edat).explosiontype >> 1;

	enemy->launchfreq = (*edat).elaunchfreq;
	enemy->launchwait = (*edat).elaunchfreq;
	enemy->launchtype = (*edat).elaunchtype % 1000;
	enemy->launchspecial = (*edat).elaunchtype / 1000;

	enemy->xaccel = (*edat).xaccel;
	enemy->yaccel = (*edat).yaccel;
	...
	if ((*edat).startxc != 0)
		enemy->ex = (*edat).startx + (mt_rand() % ((*edat).startxc * 2)) - (*edat).startxc + 1;
	else
		enemy->ex = (*edat).startx + 1;

	if ((*edat).startyc != 0)
		enemy->ey = (*edat).starty + (mt_rand() % ((*edat).startyc * 2)) - (*edat).startyc + 1;
	else
		enemy->ey = (*edat).starty + 1;

	enemy->exc = (*edat).xmove;
	enemy->eyc = (*edat).ymove;
	enemy->excc = (*edat).xcaccel;
	enemy->eycc = (*edat).ycaccel;
	...
	if ((*edat).xrev == 0)
		enemy->exrev = 100;
	else if ((*edat).xrev == -99)
		enemy->exrev = 0;
	else
		enemy->exrev = (*edat).xrev;

	if ((*edat).yrev == 0)
		enemy->eyrev = 100;
	else if ((*edat).yrev == -99)
		enemy->eyrev = 0;
	else
		enemy->eyrev = (*edat).yrev;
	...
	for (uint i = 0; i < 20; ++i)
		enemy->egr[i] = (*edat).egraphic[i];
	enemy->size = (*edat).esize;
	enemy->linknum = 0;
	enemy->edamaged = (*edat).dani < 0;
	enemy->enemydie = (*edat).eenemydie;

	enemy->freq[1-1] = (*edat).freq[1-1];
	enemy->freq[2-1] = (*edat).freq[2-1];
	enemy->freq[3-1] = (*edat).freq[3-1];

	enemy->edani   = (*edat).dani;
	enemy->edgr    = (*edat).dgr;
	enemy->edlevel = (*edat).dlevel;
	...
	int tempValue = 0;
	if ((*edat).value > 1 && (*edat).value < 10000)
	{
		switch (difficultyLevel)
		{
		case -1:
		case DIFFICULTY_WIMP:
			tempValue = (*edat).value * 0.75f;
			break;
		case DIFFICULTY_EASY:
		case DIFFICULTY_NORMAL:
			tempValue = (*edat).value;
			break;
		case DIFFICULTY_HARD:
			tempValue = (*edat).value * 1.125f;
			break;
		case DIFFICULTY_IMPOSSIBLE:
			tempValue = (*edat).value * 1.5f;
			break;
		case DIFFICULTY_INSANITY:
			tempValue = (*edat).value * 2;
			break;
		case DIFFICULTY_SUICIDE:
			tempValue = (*edat).value * 2.5f;
			break;
		case DIFFICULTY_MANIACAL:
		case DIFFICULTY_ZINGLON:
			tempValue = (*edat).value * 4;
			break;
		case DIFFICULTY_NORTANEOUS:
		case DIFFICULTY_10:
			tempValue = (*edat).value * 8;
			break;
		}
		if (tempValue > 10000)
			tempValue = 10000;
	...
	if ((*edat).armor > 0)
	{
		if ((*edat).armor != 255)
		{
			...
			case -1:
			case DIFFICULTY_WIMP:
				tempArmor = (*edat).armor * 0.5f + 1;
				break;
			case DIFFICULTY_EASY:
				tempArmor = (*edat).armor * 0.75f + 1;
				break;
			case DIFFICULTY_NORMAL:
				tempArmor = (*edat).armor;
				break;
			case DIFFICULTY_HARD:
				tempArmor = (*edat).armor * 1.2f;
				break;
			case DIFFICULTY_IMPOSSIBLE:
				tempArmor = (*edat).armor * 1.5f;
				break;
			case DIFFICULTY_INSANITY:
				tempArmor = (*edat).armor * 1.8f;
				break;
			case DIFFICULTY_SUICIDE:
				tempArmor = (*edat).armor * 2;
				break;
			case DIFFICULTY_MANIACAL:
				tempArmor = (*edat).armor * 3;
				break;
			case DIFFICULTY_ZINGLON:
				tempArmor = (*edat).armor * 4;
				break;
			case DIFFICULTY_NORTANEOUS:
			case DIFFICULTY_10:
				tempArmor = (*edat).armor * 8;
				break;
			}
...
void JE_eventSystem(void)
{
	...
	case 52:
		...
		enemyDat_scratch.armor = tempDat3;
		enemyDat_scratch.egraphic[1-1] = tempDat2;
