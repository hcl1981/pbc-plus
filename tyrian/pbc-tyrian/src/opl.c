...
/*
 * PicoBoy: 256 statt 512.
 *
 * Der Wert bestimmt vier statische Zwischenpuffer zu je BLOCKBUF_SIZE * 4
 * Byte. Bei 512 waren das 8 KB, die hier an anderer Stelle fehlen. Der
 * Tonpuffer des Geraets ist ohnehin 256 Werte gross, groessere Bloecke bringen
 * also nichts.
 */
#define BLOCKBUF_SIZE		256
...
void adlib_getsample(Bit16s* sndptr, Bits numsamples) {
	...
	/* PicoBoy: statisch statt auf dem Stapel. Diese Funktion laeuft in der
	   Ton-Unterbrechung, und 6 KB Stapel sind dort schlicht nicht da. Sie hat
	   genau einen Aufrufer und laeuft nie nebenlaeufig. */
	static Bit32s outbufl[BLOCKBUF_SIZE];
#if defined(OPLTYPE_IS_OPL3)
	// second output buffer (right channel for opl3 stereo)
	static Bit32s outbufr[BLOCKBUF_SIZE];
#endif

	// vibrato/tremolo lookup tables (global, to possibly be used by all operators)
	static Bit32s vib_lut[BLOCKBUF_SIZE];
	static Bit32s trem_lut[BLOCKBUF_SIZE];
