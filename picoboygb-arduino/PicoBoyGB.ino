...
// ===========================================================================
// Build-Time Guards: Pruefen, dass Tools-Menue korrekt gewaehlt wurde
// ---------------------------------------------------------------------------
// Diese Checks werfen mit einer deutlichen Fehlermeldung ab, wenn der User
// in der Arduino IDE das falsche Board oder die falsche Flash-Konfiguration
// gewaehlt hat. Besser ein klarer Compile-Error als ein mysterioeser Runtime-
// Haenger nach erfolgreichem Upload.
// ===========================================================================
#if !defined(PICO_RP2350)
  #error "Wrong board: this sketch is for RP2350 (PicoBoy Color). " \
         "In Arduino IDE: Tools > Board > Raspberry Pi Pico/RP2040 > Generic RP2350."
#endif

// arduino-pico definiert PICO_FLASH_SIZE_BYTES nach dem aktuell gewaehlten
// Flash-Size-Menue. Wir brauchen >= 16 MB damit unsere ROM-Region bei
// 0x10100000 (2 MB Cache) plus der FatFS-Partition (12 MB) reinpassen.
#if !defined(PICO_FLASH_SIZE_BYTES) || PICO_FLASH_SIZE_BYTES < (16 * 1024 * 1024)
  #error "Wrong flash size: select Tools > Flash Size > '16MB (Sketch: 4MB, FS: 12MB)'."
#endif

// USB-Drive-Funktionalitaet: arduino-pico exportiert eine FatFS-Partition
// als USB-Massenspeicher. Damit das Linker-Fragment fuer FatFS reserviert
// wird, muss in der IDE unter Tools > Flash Size eine FS-Groesse > 0
// gewaehlt sein, z.B. "16MB (FS: 12MB)".
#include <FatFS.h>
#include <FatFSUSB.h>
...
static void fatfs_ensure_welcome() {
    File f = FatFS.open("/README.TXT", "r");
    if (f) { f.close(); return; }
    f = FatFS.open("/README.TXT", "w");
    if (!f) return;
    f.println("PicoBoyGB - Game Boy DMG Emulator");
    f.println("");
    f.println("Drag .gb ROMs onto this drive, then EJECT it.");
    f.println("Press RESET (without holding CENTER) to return");
    f.println("to the game menu.");
    f.println("");
    f.println("Hold CENTER while pressing RESET to come back");
    f.println("to this USB stick mode.");
    f.println("");
    f.println("In the menu use UP/DOWN to navigate, A or CENTER");
    f.println("to confirm, LEFT/RIGHT to toggle Audio.");
    f.close();
}
...
static int fatfs_scan_gb_files() {
    g_rom_count = 0;
    Dir d = FatFS.openDir("/");
    while (d.next() && g_rom_count < MAX_ROMS) {
        String name = d.fileName();
        if (name.length() == 0) continue;
        // Endet auf .gb (case-insensitive)?
        char lower[MAX_ROMNAME_LEN];
        name.toCharArray(lower, sizeof(lower));
        str_lower_inplace(lower);
        size_t L = strlen(lower);
        if (L < 4) continue;
        if (strcmp(lower + L - 3, ".gb") != 0) continue;
        // Original-Name uebernehmen
        name.toCharArray(g_rom_names[g_rom_count], MAX_ROMNAME_LEN);
        ++g_rom_count;
    }
...
static uint32_t fatfs_file_hash_first4k(const char *name) {
    ...
    if (!f) return 0;
    if (f.size() < 0x150) { f.close(); return 0; }
    uint32_t h = 2166136261u;
    uint8_t buf[256];
    size_t to_check = f.size() < 4096 ? f.size() : 4096;
    size_t done = 0;
    while (done < to_check) {
        size_t n = (to_check - done) > sizeof(buf) ? sizeof(buf) : (to_check - done);
        f.read(buf, n);
...
static bool fatfs_program_rom(const char *name) {
    ...
    uint32_t erase_size = (sz + 4095) & ~4095u;
    uint32_t intr = save_and_disable_interrupts();
    ...
    while (off < sz) {
        ...
        size_t want = (sz - off < sizeof(pagebuf)) ? (sz - off) : sizeof(pagebuf);
        size_t got  = f.read(pagebuf, want);
        if (got != want) {
            f.close();
...
static void run_usb_stick_mode() {
    ...
    // freigeben damit der Host vollen Zugriff bekommt.
    bool fs_ok = FatFS.begin();
    if (!fs_ok) {
        if (FatFS.format()) fs_ok = FatFS.begin();
    }
    if (fs_ok) {
        ...
        // Keine FS-Partition -> Hinweis im GB-Bereich.
        gb_screen_message("NO FS PARTITION", "Set Tools > Flash", "to include FS.");
        while (true) tight_loop_contents();
    }

    // Endlos warten bis User Power-Cycle macht.
    while (true) tight_loop_contents();
}
...
void setup() {
    ...
    if (fs_ok) {
        ...
        // Keine FatFS-Partition vorhanden -> Hinweis im GB-Bereich, halt.
        gb_screen_message("NO FS PARTITION", "Set Tools > Flash", "to include FS.");
        while (true) tight_loop_contents();
    }

    // FatFSUSB hier NICHT starten - wir wollen nur emulieren, nicht parallel
    // Drive sein.
