...
void pbc_audio_frame(const stb_audio_t *a)
{
    uint32_t rd;
    int room;
    ...
    /* Schubduese: Rauschen, solange Schub liegt.  Die Vorlage spielt dafuer
     * eine Schleife aus einer WAV-Datei; hier wird es erzeugt. */
    thrust_target = a->thrust ? 40 : 0;

    if (a->ev & EV_DIE) {
        static const uint16_t hz[4] = { 262, 196, 147, 98 };
        static const uint8_t  ms[4] = { 90, 90, 120, 250 };
        play(hz, ms, 4);
        crackle_amp = 120;
    } else if (a->ev & EV_LEVELDONE) {
        static const uint16_t hz[4] = { 523, 659, 784, 1047 };
        static const uint8_t  ms[4] = { 80, 80, 80, 180 };
        play(hz, ms, 4);
    } else if (a->ev & EV_TAKEBALL) {
        static const uint16_t hz[3] = { 440, 660, 880 };
        static const uint8_t  ms[3] = { 60, 60, 110 };
        play(hz, ms, 3);
    } else if (a->ev & EV_EXPLODE) {
        note(147, 90, 34);
        if (crackle_amp < 90) crackle_amp = 90;
    } else if (a->ev & EV_SWITCH) {
        note(1175, 60, 28);
    } else if (a->ev & EV_UI) {
        note(880, 45, 26);
    } else if (a->ev & EV_FUEL) {
        note(1568, 30, 18);
    } else if (a->ev & EV_HIT) {
        if (crackle_amp < 45) crackle_amp = 45;
    }
    if (a->ev & EV_SHOT) {
        if (crackle_amp < 30) crackle_amp = 30;
    }
    if (a->ev & EV_ENEMYSHOT) {
        if (crackle_amp < 22) crackle_amp = 22;
    }
