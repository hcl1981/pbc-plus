...
int main(void)
{
    ...
    stb_init(0xA5A5A5A5u, 0);
    stb_render_init();
    stb_render.show_debug = 1;
    ...
    for (;;) {
        ...
        if (page == 4) {
            ...
            int cur = 0, y;
            stb_tick((uint16_t)(pad | PAD_B));
            stb_render.blink++;
            a = stb.audio;
            t1 = time_us_64();
            stb_render.ms_cpu = (int)((t1 - t0) / 1000u);
            pbc_display_window(0, 0, SCR_W, SCR_H);
            pbc_display_begin();
            for (y = 0; y < SCR_H; y += 8) {
                stb_render_rows(strip[cur], y, 8);
                pbc_display_wait();
            ...
            if (page == 3) {
                if (held & PAD_B) a.thrust = 1;
                if (pad & TRG_A)  a.ev = EV_EXPLODE;
            }
        }
