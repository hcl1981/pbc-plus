...
void pbc_audio_frame(const spout_audio_t *a)
{
    uint32_t rd;
    int room;
    ...
    thrust_target = a->thrust ? 46 : 0;

    if (a->hits) {
        int add = (int)a->hits * 5;
        if (add > 70) add = 70;
        if (add > crackle_amp) crackle_amp = add;
    }
    if (a->breaks) {
        int add = 60 + (int)a->breaks * 8;
        if (add > 110) add = 110;
        if (add > crackle_amp) crackle_amp = add;
    }

    if (a->ev & SPOUT_EV_OVER) {
        static const uint16_t hz[4] = { 392, 330, 262, 196 };
        static const uint8_t  ms[4] = { 110, 110, 110, 220 };
        play(hz, ms, 4);
    } else if (a->ev & SPOUT_EV_BONUS) {
        static const uint16_t hz[3] = { 659, 880, 1319 };
        static const uint8_t  ms[3] = { 60, 60, 110 };
        play(hz, ms, 3);
    } else if (a->ev & SPOUT_EV_START) {
        static const uint16_t hz[2] = { 523, 784 };
        static const uint8_t  ms[2] = { 70, 120 };
        play(hz, ms, 2);
    } else if (a->ev & SPOUT_EV_BARRIER) {
        static const uint16_t hz[2] = { 175, 131 };
        static const uint8_t  ms[2] = { 90, 130 };
        play(hz, ms, 2);
    } else if (a->ev & SPOUT_EV_TICK) {
        note(1245, 40, 30);
    } else if (a->ev & SPOUT_EV_UI) {
        note(880, 45, 28);
    }
