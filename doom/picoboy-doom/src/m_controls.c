...
#ifdef PICOBOY_BUILD
// PicoBoy: bind KEY_INS to "next weapon" so our A button (which we map
// to KEY_INS in i_input_picoboy.c) drives the engine's built-in
// next-weapon cycle. That cycle automatically skips weapons the player
// doesn't own and handles the slot-1 fist/chainsaw split correctly --
// much cleaner than emulating digit-key presses ourselves.
key_type_t key_nextweapon = KEY_INS;
#else
