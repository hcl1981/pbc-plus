...
#if DOOM_TINY
    // Die Statusleisten-Anzeigen werden in ST_Init einmalig an players[0]
    // gebunden ("one time init anyway"), und ST_Start(), das sie sonst bei
    // jedem Levelstart neu binden wuerde, ruft in diesem Port niemand mehr auf.
    // Im Einzelspiel faellt das nicht auf, weil consoleplayer dort immer 0 ist.
    // Im Netzspiel ist der Beitretende aber Spieler 1 -- und sah dadurch
    // dauerhaft die Werte des HOSTS: dessen Gesundheit, Ruestung, Munition,
    // Waffen und Schluessel. Deshalb blieb seine Gesundheit auf 100, waehrend
    // er selbst getroffen wurde und starb.
    // Geprueft wird die Bindung der Anzeige selbst, nicht nur der Spieler:
    // das Gesicht liest direkt ueber plyr und ist deshalb schon richtig,
    // sobald plyr stimmt -- die Zahlen haengen dagegen an Zeigern, die
    // ausschliesslich ST_createWidgets() setzt. Genau diese beiden konnten
    // auseinanderlaufen: Gesicht reagierte, Gesundheit blieb auf 100.
    if (plyr != &players[consoleplayer] || w_health.n.num != &plyr->health) {
        plyr = &players[consoleplayer];
        ST_createWidgets();
    }
    // Und zur Sicherheit bei jedem Bild neu verknuepfen -- genau so macht es
    // die Munitionsanzeige ohnehin (w_ready.num wird in ST_updateWidgets jeden
    // Tic neu gesetzt), und die war als einzige von Anfang an richtig. Damit
    // kann keine Anzeige mehr an einem veralteten Spieler haengenbleiben.
