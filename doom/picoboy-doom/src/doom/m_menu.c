...
static const menuitem_t NetGameMenu[]=
        {
                // Namenseingabe entfaellt: ohne Tastatur nicht bedienbar, die
                // Namen vergibt jetzt die Rolle (PLAYER 1 / PLAYER 2).
...
void M_DrawNetFoyer(void) {
    ...
    if (ls.status == lobby_no_connection) {
        ...
        // Am Kabel haengen genau zwei Geraete -- vier Plaetze anzuzeigen war nur
        // aus der Netzwerkfassung uebernommen.
        for(int i=0;i<2;i++) {
            sprintf(buf, "%d.", i+1);
...
{
    if (M_CheckNetGame()) {
        // Ohne Tastatur kann man den Namen nicht sinnvoll aendern -- also
        // vergibt die Rolle ihn: Host = PLAYER 1, Beitretender = PLAYER 2.
