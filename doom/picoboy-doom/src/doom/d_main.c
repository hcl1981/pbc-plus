...
#ifdef PICOBOY_BUILD
// Defined in src/pico/picoboy_diag.c -- sets the 3 diag LEDs (red/yellow/
// green) to the binary pattern of n (1..7). Used as progress markers in
// this file (D_DoomMain milestones, D_DoomLoop / D_RunFrame / D_Display).
extern void picoboy_mark(int n);
#endif
...
boolean D_Display (void)
{
    ...
    {
#ifdef PICOBOY_BUILD
	// PicoBoy: skip the screen wipe / melt transition. The wipe code
	// in pd_render.cpp depends on tightly synchronised scanvideo
	// double-buffer flips that we don't have, so triggering it would
	// cause visible buffer-mismatch glitches (e.g. parts of the title
	// screen showing under the status bar, or duplicated rows when
	// entering / leaving levels). Hard cut between game states is
	// barely noticeable on the small display.
	wipe = false;
#else
	wipe = true;
#if !DOOM_TINY
...
#endif
    I_InitGraphics();
#if USB_SUPPORT && !defined(PICOBOY_BUILD)
...
//    W_PrintDirectory();
    ...
    I_InitJoystick();
#endif
    // PicoBoy: SFX enabled; OPL music is stubbed out in I_OPL_InitMusic
    // (returns false) so no OPL is initialised. I_InitMusic() is empty.
    I_InitSound(true);
    I_InitMusic();
