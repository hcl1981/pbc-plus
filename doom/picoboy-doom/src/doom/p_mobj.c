...
void P_SpawnPlayer (const mapthing_t* mthing)
{
...
#ifdef PICOBOY_BUILD
	// PicoBoy: cheats selected by holding A and/or B at power-on.
	// Re-applied on every spawn so they persist across levels/restarts.
	// (iddqd = god mode, idkfa = full arsenal + keys + ammo + armor.)
	extern boolean picoboy_boot_god;
	extern boolean picoboy_boot_kfa;
	if (picoboy_boot_god)
	{
	    p->cheats |= CF_GODMODE;
	    p->health = 100;
	...
	if (picoboy_boot_kfa)
	{
	    p->armorpoints = 200;   // idkfa armor
	    p->armortype = 2;       // blue armor class
