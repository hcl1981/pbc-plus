...
#ifdef PICOBOY_BUILD
extern "C" void picoboy_mark(int);
extern "C" void picoboy_finish_update_export(void);
#endif
...
#ifdef PICOBOY_BUILD
                // We have no scanvideo overlay path, so always render the
                // intermission patches (numbers, level names, etc.) directly
                // into the framebuffer. Without this, WI_Drawer's output
                // would go into an overlay that never gets composited.
                //
                // Repaint the background every frame too, so old patches
                // (e.g. the "kills/items/secrets" stats) are erased before
                // the next intermission sub-stage ("Entering Nuclear Plant")
                // is drawn over the top.
                if (!wipestate) {
                    if (wi_background_patch_num) {
                        draw_splash(wi_background_patch_num, 0, MAIN_VIEWHEIGHT,
                                    frame_buffer[render_frame_index]);
                        draw_splash(wi_background_patch_num, MAIN_VIEWHEIGHT, SCREENHEIGHT,
                                    frame_buffer[render_frame_index ^ 1]
                                        + (MAIN_VIEWHEIGHT - 32) * SCREENWIDTH);
                    }
                    V_BeginPatchList(vpatchlists->framebuffer);
                    WI_Drawer();
                    draw_framebuffer_patches_fullscreen();
...
#ifdef PICOBOY_BUILD
    // In the original, scanvideo on core1 ran continuously and called
    // new_frame_stuff() once per displayed frame -- that swapped
    // display_frame_index and recycled display_frame_freed permits.
    // We have no scanvideo, so drive the per-frame swap + panel update
    // ourselves right here.

    // Force next_video_type from the current gamestate. The original
    // code only re-synced this inside the wipe handler, which we have
    // disabled -- so without this line, next_video_type can stay stuck
    // on VIDEO_TYPE_DOUBLE after exiting a level (causing the status
    // bar to be drawn over the intermission stats screen) or on
    // VIDEO_TYPE_SINGLE on entering a level.
    next_video_type = (gamestate == GS_LEVEL) ? VIDEO_TYPE_DOUBLE
                                              : VIDEO_TYPE_SINGLE;

    // In gameplay (VIDEO_TYPE_DOUBLE) the engine relies on scanvideo
    // overlays to draw the status bar; since we don't have scanvideo,
    // render it into the frame_buffer's tail rows ourselves so our
    // compose pipeline picks it up. We render into `render_frame_index ^ 1`
    // because Doom keeps the bar in its "fb_b" slot (the slot opposite
    // the main view), and the engine has just swapped render_frame_index
    // a few lines above.
    if (next_video_type == VIDEO_TYPE_DOUBLE) {
        draw_stbar_on_framebuffer(render_frame_index ^ 1, true);
    }
    picoboy_finish_update_export();
#endif
}
