...
{
#ifdef PICOBOY_BUILD
    // PicoBoy: piezo speaker can't reproduce OPL music adequately, and
    // the actual OPL init path tries to load GENMIDI from the WAD and
    // configure I2S audio drivers we don't have. Just claim "no music".
    // music_module stays NULL in i_sound.c and every later music call
    // becomes a no-op.
    return false;
#endif
#if !DOOM_TINY
