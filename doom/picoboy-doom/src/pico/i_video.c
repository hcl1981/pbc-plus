...
// PicoBoy-specific hooks. Implementations live further down this file
// (picoboy_finish_update) and in i_input_picoboy.c (picoboy_input_init).
extern void picoboy_input_init(void);
static void picoboy_finish_update(void);
extern void picoboy_finish_update_export(void);
extern void picoboy_mark(int n);   // src/pico/picoboy_diag.c
#endif
...
{
#ifdef PICOBOY_BUILD
    // PicoBoy: bring display + input up before any of the engine's own
    // graphics init (semaphores, pd_init, multicore launch).
...
#ifdef PICOBOY_BUILD
    extern void picoboy_input_poll(void);
    picoboy_input_poll();
#endif
...
#ifdef PICOBOY_BUILD
// ---------------------------------------------------------------------------
// PicoBoy Color: per-frame display push.
//
// Replaces the original VGA scanvideo path. By the time we get here, the
// renderer has finished writing into frame_buffer[next_frame_index] and has
// signalled render_frame_ready. We:
//   1. Run the same swap-and-overlays bookkeeping that the original
//      scanvideo loop did once per frame (new_frame_stuff).
//   2. Walk frame_buffer[display_frame_index] (3D view) plus
//      frame_buffer[display_frame_index ^ 1] (status bar lives there at
//      offset (line - 32) * 320), convert through the active 256-entry
//      palette to RGB565, decimate 320 -> 240 horizontally and 200 -> 180
//      vertically, into picoboy_display.c's framebuffer.
//   3. DMA the framebuffer to the ST7789.
// ---------------------------------------------------------------------------
#if USE_PICO_NET
#include "piconet.h"
#endif
#if PICONET_DEBUG_SCREEN && USE_PICO_NET
#include "usblink.h"
// Link-Diagnose in den schwarzen Streifen unter dem Bild -- nur sichtbar,
// solange ein Netzwerkspiel aufgebaut ist oder laeuft.
static void picoboy_draw_net_status(void)
{
    static bool was_active;
    int line = piconet_debug_line_state();
    if (line < 0) {
        if (was_active) {
            // Der Link war schon aktiv und ist jetzt abgeraeumt -- das ist die
            // interessanteste Meldung ueberhaupt, die bleibt stehen.
            char off[24];
            snprintf(off, sizeof(off), "LINK OFF R%u/%u",
                     (unsigned)piconet_dbg_stop_reason, (unsigned)piconet_dbg_stop_info);
            picoboy_status_lines(off, "1=TOT 2=SPIELER");
            return;
        }
        // Kein Netzspiel: die Tic-Rate trotzdem zeigen. Damit laesst sich
        // vergleichen, was das Spiel allein schafft -- und was die Verbindung
        // wirklich kostet.
        {
            extern int gametic;
            static uint32_t sp_t0, sp_base;
            static char sp[24];
            uint32_t now_sp = time_us_32();
            if (now_sp - sp_t0 >= 1000000) {
                snprintf(sp, sizeof(sp), "D%lu",
                         (unsigned long)((uint32_t)(gametic - (int)sp_base) % 100));
                sp_base = (uint32_t)gametic;
                sp_t0 = now_sp;
            }
            if (sp[0]) picoboy_status_lines(sp, "");
        }
        return;
    }
    was_active = true;

    // Nicht in jedem Frame neu schreiben: der Streifen kostet ~2,5 ms SPI und
    // die Zahlen sollen lesbar sein, nicht flackern.
    static uint32_t next_update;
    uint32_t now = time_us_32();
    if ((int32_t)(now - next_update) < 0) return;
    next_update = now + 400000;

    char l1[24], l2[24];
    // Haeufigster Fehler statt des letzten: der letzte ist meist 0 und sagt
    // nichts darueber, woran die Versuche eigentlich scheitern.
    unsigned top_code = 0, top_count = 0;
    for (unsigned i = 1; i < USBLINK_FAIL_CODES; i++) {
        if (usblink_fail_counts[i] > top_count) {
            top_count = usblink_fail_counts[i];
            top_code = i;
        }
    }
    if (piconet_debug_is_host()) {
        snprintf(l1, sizeof(l1), "HOST L%d F%u:%u H%lu", line, top_code,
                 top_count % 1000, (unsigned long)usblink_get_speed());
    } else {
        // Der Client taktet nicht -- eine Rate anzuzeigen waere frei erfunden.
        snprintf(l1, sizeof(l1), "JOIN L%d F%u:%u", line, top_code, top_count % 1000);
    }
    int own_tic, remote_tic;
    if (piconet_debug_tics(&own_tic, &remote_tic)) {
        // Im Spiel: kommen die Tics voran? Bleibt R stehen, waehrend T laeuft,
        // liefert die Gegenstelle nichts mehr.
        // X = gelungene Austausche je Sekunde. Doom braucht 35 Tics/s, und
        // jeder Austausch traegt einen -- diese Zahl entscheidet ueber das Tempo.
        // D = Tics je Sekunde (Doom braucht 35), X = Austausche je Sekunde,
        // E = Fehler. D ist die Zahl, die ueber "zaeh" oder "fluessig"
        // entscheidet -- alles andere ist nur Mittel zum Zweck.
        static uint32_t rate_t0, rate_base, rate_show, tic_base, tic_show;
        if (now - rate_t0 >= 1000000) {
            rate_show = usblink_stat_ok - rate_base;
            rate_base = usblink_stat_ok;
            tic_show = (uint32_t)(own_tic - (int)tic_base);
            tic_base = (uint32_t)own_tic;
            rate_t0 = now;
        }
        snprintf(l2, sizeof(l2), "D%lu X%lu E%lu",
                 (unsigned long)(tic_show % 100), (unsigned long)rate_show,
                 (unsigned long)((usblink_stat_timeout + usblink_stat_badframe) % 1000));
    } else {
        snprintf(l2, sizeof(l2), "OK%lu E%lu F%u/%u",
                 (unsigned long)(usblink_stat_ok % 1000),
                 (unsigned long)((usblink_stat_timeout + usblink_stat_badframe) % 1000),
                 (unsigned)usblink_fail_code, (unsigned)usblink_fail_pos);
    }
    picoboy_status_lines(l1, l2);
}
#endif

// Bild auslassen, wenn die Spiellogik hinterherhinkt.
//
// pd_end_frame() -- und damit dieser Aufruf -- laeuft auf Core0, genau dort,
// wo auch die Tics entstehen. Ein ausgelassenes Bild gibt dem Spiel also
// unmittelbar Rechenzeit zurueck.
//
// Eigene Zeitmessung, bewusst NICHT I_GetTime(): das rechnet in diesem Port
// um Faktor 1000 falsch. Hier wird ueber jedes Bild verglichen, wie viele Tics
// haetten entstehen muessen und wie viele wirklich entstanden sind.
static bool picoboy_should_skip_frame(void)
{
#if PICOBOY_FRAME_SKIP
    extern int gametic;
    static uint32_t last_us;
    static int last_gametic;
    static int debt;
    static bool skipped_last;

    uint32_t now = time_us_32();
    uint32_t dt = now - last_us;
    if (dt > 500000) dt = 500000;      // nach langen Pausen (Levelladen) nichts nachtragen
    int expected = (int)((uint64_t)dt * TICRATE / 1000000u);
    int done = gametic - last_gametic;
    last_us = now;
    ...
    // Nie zwei Bilder hintereinander auslassen -- das saehe stockend aus.
    if (debt >= 2 && !skipped_last) {
        skipped_last = true;
        debt -= 1;
        return true;
    }
...
#if USE_PICO_NET
#include "piconet.h"
// Bleibt der Tic-Fluss stehen, warten beide Geraete fuer immer aufeinander --
// das Spiel sieht dann aus, als haenge es. Das wird jetzt angezeigt, und nach
// einigen Sekunden loest sich die Verbindung selbst auf, damit man
// weiterspielen kann statt zum Reset greifen zu muessen.
static void picoboy_check_net_stall(void)
{
    static uint32_t last_shown;
    uint32_t stalled = piconet_stalled_ms();
    if (stalled < 1500) {
        if (last_shown) {
            picoboy_status_lines("", "");
            last_shown = 0;
        }
        return;
    }
    if (stalled >= 6000) {
        // Aufgeben und alleine weiterspielen -- besser als ein totes Bild.
        extern boolean net_client_connected;
        piconet_dbg_stop_reason = 5;
        net_client_connected = false;
        piconet_stop();
        picoboy_status_lines("VERBINDUNG WEG", "ALLEINE WEITER");
        last_shown = 1;
        return;
    }
    char l[24];
    snprintf(l, sizeof(l), "SEIT %lu SEKUNDEN", (unsigned long)(stalled / 1000));
    picoboy_status_lines("WARTE AUF MITSPIELER", l);
    last_shown = 1;
}
#endif
...
static void picoboy_finish_update(void)
{
    new_frame_stuff();
#if USE_PICO_NET
    picoboy_check_net_stall();
#endif

    if (picoboy_should_skip_frame()) {
        // Der Renderer bekommt seinen Puffer trotzdem zurueck (new_frame_stuff
        // oben) -- nur die Ausgabe entfaellt.
        return;
    }


    picoboy_compose_frame_doom(
        frame_buffer[display_frame_index],
        frame_buffer[display_frame_index ^ 1],
        palette);

    picoboy_present();

#if PICONET_DEBUG_SCREEN && USE_PICO_NET
    picoboy_draw_net_status();
#endif
}

// Non-static wrapper so pd_render.cpp (C++) can call it via extern "C".
void picoboy_finish_update_export(void)
{
    picoboy_finish_update();
}
#endif // PICOBOY_BUILD
